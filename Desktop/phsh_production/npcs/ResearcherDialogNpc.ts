// NOTE: When instantiating AncientToolmasterDialogNpc, pass the config loaded from '../data/npc_definitions/researcher.json'.
...
// Define all craftable items at the Ancient Toolmaster Forge
...
export class ResearcherDialogNpc extends DialogNpc {
    private static ANCIENT_TOOLMASTER_CRAFTABLES: CraftableItemEntry[] = [
        // Basic offerings (same as original Toolmaster)
        {
            itemId: 'iron_nugget',
            materials: [
        ...
        // New lure offerings
        {
            itemId: 'bait_carved_lure',
            materials: [
                { id: 'driftwood', name: 'Driftwood', required: 1 },
                { id: 'iron_nugget', name: 'Iron Nugget', required: 1 }
            ],
            coinCost: 30,
            category: 'materials'
        },
        {
            itemId: 'bait_ancient_lure',
            materials: [
                { id: 'ancient_driftwood', name: 'Ancient Driftwood', required: 1 },
                { id: 'iron_nugget', name: 'Iron Nugget', required: 1 }
            ],
            coinCost: 75,
            category: 'materials'
        ...
        // Gold Scarab Bait
        {
            itemId: 'bait_gold_scarab',
            materials: [
                { id: 'gold_nugget', name: 'Gold Nugget', required: 1 },
                { id: 'scarab', name: 'Scarab', required: 1 }
            ],
            coinCost: 100,
            category: 'materials'
        }
    ...
    constructor(
        ...
        console.log(`[AncientToolmasterDialogNpc ${this.config.id}] Initialized.`);
    }

    // --- Override spawn to set idle_upper and idle_lower animations ---
    public override spawn(): void {
        // Call parent spawn first
        super.spawn();

        // After spawning, set idle_upper and idle_lower animations (like greeter NPCs)
        if (this.entity) {
    ...
    protected getSeaStoryOptions(story: SeaStory): string[] {
        return this.config.interaction?.options?.map(opt => opt.text) || ["Open Forge", "Who are you?"];
    }
    ...
    public override handleOption(player: Player, option: number): void {
        ...
        if (!playerUI || !state) {
            console.warn(`[AncientToolmasterDialogNpc ${this.config.id}] Received option ${option} from non-interacting player ${player.id}.`);
            return;
        }
        ...
        if (option === 0) {
            ...
            // Send craftable items data to client
        ...
        if (playerUI.dialog) {
            try { 
                ...
                console.error(`[AncientToolmasterDialogNpc ${this.config.id}] Error updating dialog for player ${player.id}:`, e);
            }
        } else {
            console.warn(`[AncientToolmasterDialogNpc ${this.config.id}] No dialog UI found for player ${player.id}`);
        }
    ...
    private showToolmasterForge(player: Player) {
        ...
        let craftableItems = [];
        for (const craftableEntry of ResearcherDialogNpc.ANCIENT_TOOLMASTER_CRAFTABLES) {
            let itemData = null;
